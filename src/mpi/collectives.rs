use crate::{types::*, private::*, MPI_CHECK, MPI_Comm_size, MPI_Comm_rank, MPI_Send, MPI_Recv, uninit, MPI_CHECK_OP, MPI_CHECK_TYPE, MPI_Comm_call_errhandler, MPI_Type_size};
...
#[no_mangle]
pub extern "C" fn MPI_Bcast(buf : *mut c_void, cnt : i32, dtype : MPI_Datatype, mut root : i32, comm : MPI_Comm) -> i32 {
        ...
        loop {
            ...
            if Context::rank() == root {
                if diff + n < Context::size() {
                    ...
                    code = MPI_Send(buf, cnt, dtype, (Context::rank() + n) % Context::size(), BCAST_TAG, comm);
                    if code != MPI_SUCCESS {
                        break;
                    }
                }
...
#[no_mangle]
pub extern "C" fn MPI_Reduce(sbuf : *const c_void,mut rbuf : *mut c_void, cnt : i32, dtype : MPI_Datatype, op : MPI_Op, root : i32, comm : MPI_Comm) -> i32 {
    ...
    if Context::size() == 2 {
        if Context::rank() == root {
            code = MPI_Recv(rbuf, cnt, dtype, (root + 1) % 2, REDUCE_TAG, comm, &mut stat);
            if code == MPI_SUCCESS {
        ...
        if diff % 2 != 0 {
            debug!("Send to {}", (Context::size() + Context::rank() - 1) % Context::size());
            code = MPI_Send(sbuf, cnt, dtype, (Context::size() + Context::rank() - 1) % Context::size(), REDUCE_TAG, comm);
        } else if diff < (Context::size() - 1) {
            debug!("Recv from {}", (Context::rank() + 1) % Context::size());
            code = MPI_Recv(rbuf, cnt, dtype, (Context::rank() + 1) % Context::size(), REDUCE_TAG, comm, &mut stat);
            if code == MPI_SUCCESS {
            ...
            if diff % 4 != 0 {
                if diff % 2 == 0 {
                    if diff < Context::size() - 1 {
                        debug!("Write to {}", (Context::size() + Context::rank() - 2) % Context::size());
                        code = MPI_Send(rbuf, cnt, dtype, (Context::size() + Context::rank() - 2) % Context::size(), REDUCE_TAG, comm);
                    } else {
                        debug!("Write to {}", (Context::size() + Context::rank() - 2) % Context::size());
                        code = MPI_Send(sbuf, cnt, dtype, (Context::size() + Context::rank() - 2) % Context::size(), REDUCE_TAG, comm);
                    }
                }
                ...
                code = MPI_Recv(tbuf, cnt, dtype, (Context::rank() + 2) % Context::size(), REDUCE_TAG, comm, &mut stat);
                if code == MPI_SUCCESS {
                    debug!("Do function");
            ...
            if code == MPI_SUCCESS {
                ...
                while iold < Context::size() {
                    if diff % i != 0 {
                        if diff % iold == 0 {
                            if diff < Context::size() - 1 {
                                debug!("Send to: {}", (Context::size() + Context::rank() - iold) % Context::size());
                                code = MPI_Send(rbuf, cnt, dtype, (Context::size() + Context::rank() - iold) % Context::size(), REDUCE_TAG, comm);
                            } else {
                                debug!("Send to: {}", (Context::size() + Context::rank() - iold) % Context::size());
                                code = MPI_Send(sbuf, cnt, dtype, (Context::size() + Context::rank() - iold) % Context::size(), REDUCE_TAG, comm);
                            }
                        }
                    } else if diff < Context::size() - iold {
                        debug!("Recv from: {}", (Context::rank() + iold) % Context::size());
                        code = MPI_Recv(tbuf, cnt, dtype, (Context::rank() + iold) % Context::size(), REDUCE_TAG, comm, &mut stat);
                        if code == MPI_SUCCESS {
                            FUNCTIONS[op as usize](tbuf, rbuf, cnt, dtype);
                        }
                    }
...
#[no_mangle]
pub extern "C" fn MPI_Gather(sbuf : *const c_void, scnt : i32, sdtype : MPI_Datatype, mut rbuf : *mut c_void, rcnt : i32, rdtype : MPI_Datatype, root : i32, comm : MPI_Comm) -> i32 {
    ...
    if Context::size() == 2 {
        if Context::rank() == root {
            ...
            code = MPI_Recv(unsafe {rbuf.add((rext * ((root + 1) % 2)) as usize)}, rcnt, rdtype, (root + 1) % 2, GATHER_TAG, comm, &mut stat);
            if code == MPI_SUCCESS {
                if p_mpi_type_copy2(unsafe {rbuf.add((rext * root) as usize)}, rcnt, rdtype, sbuf, scnt, sdtype).is_null() {
                    p_mpi_dec_key(comm);
                    MPI_Comm_call_errhandler(comm, MPI_ERR_INTERN);
                    return MPI_ERR_INTERN;
                }
        ...
        if Context::rank() == root {
            ...
            if p_mpi_type_copy2(unsafe {rbuf.add((rext * Context::rank()) as usize)}, rcnt, rdtype, sbuf, scnt, sdtype).is_null() {
                p_mpi_dec_key(comm);
                MPI_Comm_call_errhandler(comm, MPI_ERR_INTERN);
                return MPI_ERR_INTERN;
            }

            code = MPI_Send(rbuf, rcnt * Context::size(), rdtype, (Context::rank() + 1) % Context::size(), GATHER_TAG, comm);
            if code == MPI_SUCCESS {
                code = MPI_Recv(rbuf, rcnt * Context::size(), rdtype, (Context::size() + Context::rank() - 1) % Context::size(), GATHER_TAG, comm, &mut stat);
            ...
            let layout = unsafe {Layout::from_size_align_unchecked((sext * Context::size()) as usize, 1)};
            rbuf = unsafe {std::alloc::alloc(layout)} as *mut c_void;
            if rbuf.is_null() {
                p_mpi_dec_key(comm);
                MPI_Comm_call_errhandler(comm, MPI_ERR_INTERN);
            ...
            code = MPI_Recv(rbuf, scnt * Context::size(), sdtype, (Context::size() + Context::rank() - 1) % Context::size(), GATHER_TAG, comm, &mut stat);
            if code == MPI_SUCCESS {
                if p_mpi_type_copy(unsafe {rbuf.add((sext * Context::rank()) as usize)}, sbuf, scnt, sdtype).is_null() {
                    p_mpi_dec_key(comm);
                    MPI_Comm_call_errhandler(comm, MPI_ERR_INTERN);
                    return MPI_ERR_INTERN;
                }
                code = MPI_Send(rbuf, scnt * Context::size(), sdtype, (Context::rank() + 1) % Context::size(), GATHER_TAG, comm);
            }
            unsafe {
                std::alloc::dealloc(rbuf as *mut u8, layout);
            }
