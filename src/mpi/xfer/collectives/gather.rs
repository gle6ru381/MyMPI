...
fn gather_ring(sbuf: &[u8], rbuf: &mut [u8], root: i32, comm: MPI_Comm) -> MpiResult {
    ...
    if csize == 2 {
        if rank == root {
            ...
            recv(&mut rbuf[offset..offset + blk_size], (root + 1) % 2, GATHER_TAG, comm, Some(&mut stat))?;
        ...
        if Context::rank() == root {
            memcpy_slice(&mut rbuf[blk_size * Context::rank() as usize..], sbuf, blk_size);
            send(rbuf, (rank + 1) % csize, GATHER_TAG, comm)?;
            recv(rbuf, (csize + rank - 1) % csize, GATHER_TAG, comm, Some(&mut stat))?;
        } else {
            let buf = crate::buffer::DynBuffer::new(rbuf.len());
            recv(buf.to_slice(), (csize + rank - 1) % csize, GATHER_TAG, comm, Some(&mut stat))?;
            memcpy_slice(&mut buf.to_slice()[blk_size * rank as usize..], sbuf, blk_size);
